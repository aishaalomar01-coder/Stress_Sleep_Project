...
-   Mean sleep duration: **`r round(mean(clean_data$sleep_duration), 2)` hours** (SD = `r round(sd(clean_data$sleep_duration), 2)`)

-   Mean stress level: **`r round(mean(clean_data$stress_level), 2)`** (SD = `r round(sd(clean_data$stress_level), 2)`)

-   Mean age: **`r round(mean(clean_data$age), 2)` years**
...
The data was taken from Kaggle, Sleep Health and Lifestyle Dataset: https://www.kaggle.com/datasets/uom190346a/sleep-health-and-lifestyle-dataset
